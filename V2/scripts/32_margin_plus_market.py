"""
Stage 32: Margin regression + isotonic + full market blend + injuries.

Combines Stage 31's margin-regression idea with Stage 29's market-blend +
injury-penalty machinery. Goal: beat 0.12287 local Brier (our current
LR+XGB blend pipeline).

Pipeline (men):
  1. Apply injury Elo penalty to 2026 team Elos (same as Stage 29).
  2. XGBRegressor predicts margin; LOSO OOF margins are isotonic-calibrated
     to probabilities.
  3. For 2026 games: predict margin, run isotonic to get p_base.
  4. Apply Stage 29's tiered market blend:
       - Tier 1 (Vegas):        alpha*p_base + (1-alpha)*p_vegas,   alpha=0.20
       - Tier 2 (Bradley-Terry): alpha*p_base + (1-alpha)*p_bt,      alpha=0.75

Women: margin regression + isotonic only (no market data).
"""
import sys
...
def main():
    log("=== STAGE 32: MARGIN + ISOTONIC + MARKET BLEND + INJURIES ===")
    ...
    # Apply market blend (Tier 1 Vegas + Tier 2 Bradley-Terry)
    ...
    print("\n" + "="*60)
    print("STAGE 32 — margin + isotonic + market blend + injuries")
    print("="*60)
    ...
    out = all_sub[["ID", "Pred"]]
    out_path = SUBMISSIONS / "v2_margin_plus_market.csv"
