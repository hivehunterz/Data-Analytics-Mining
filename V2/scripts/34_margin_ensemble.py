"""
Stage 34: Margin regression + isotonic + LOSO ENSEMBLE + market + injuries.

Difference from Stage 32: instead of training a single final XGBRegressor
on all seasons, we KEEP all LOSO-trained models and average their
predictions on 2026. Each model saw N-1 seasons — averaging reduces
variance from season-specific quirks.
"""
import sys
...
def loso_ensemble(train_df, diff_cols):
    """Leave-one-season-out: train one model per held-out season.
    Returns:
      oof: array of OOF margins (aligned to train_df) for isotonic fit
      models: list of (model, imputer) pairs for ensemble prediction
    """
...
def ensemble_predict(models, X):
    """Average predictions across all LOSO models."""
    preds = np.zeros(len(X))
    for model, imp in models:
        Xt = imp.transform(X)
...
def main():
    log("=== STAGE 34: MARGIN + ISOTONIC + LOSO ENSEMBLE + MARKET + INJURIES ===")
    ...
    log("Men: running LOSO margin regression (ensemble mode)...")
    oof_m, models_m = loso_ensemble(train_m, diff_cols_m)
    log(f"Men: {len(models_m)} LOSO models retained for ensemble")

    iso_m = IsotonicRegression(y_min=0.01, y_max=0.99, out_of_bounds="clip")
    iso_m.fit(oof_m, train_m["label"].values)
    log(f"Men OOF Brier (isotonic on margin): "
        f"{brier(train_m['label'].values, iso_m.predict(oof_m)):.5f}")

    sub_m_feat = build_submission_matchups(sub_m, m_feat, m_features, seeds_df=m_seeds)
    X_sub_m = sub_m_feat[diff_cols_m].values
    margin_pred_m = ensemble_predict(models_m, X_sub_m)
    ...
    log("Women: running LOSO margin regression (ensemble mode)...")
    oof_w, models_w = loso_ensemble(train_w, diff_cols_w)
    log(f"Women: {len(models_w)} LOSO models retained for ensemble")

    iso_w = IsotonicRegression(y_min=0.01, y_max=0.99, out_of_bounds="clip")
    iso_w.fit(oof_w, train_w["label"].values)
    log(f"Women OOF Brier (isotonic on margin): "
        f"{brier(train_w['label'].values, iso_w.predict(oof_w)):.5f}")

    sub_w_feat = build_submission_matchups(sub_w, w_feat, w_features, seeds_df=w_seeds)
    X_sub_w = sub_w_feat[diff_cols_w].values
    margin_pred_w = ensemble_predict(models_w, X_sub_w)
    ...
    print("\n" + "="*60)
    print("STAGE 34 — margin + isotonic + LOSO ENSEMBLE + market + injuries")
    print("="*60)
    print(f"  Men   (n={nm}): Brier = {men_b:.5f}")
    print(f"  Women (n={nw}): Brier = {women_b:.5f}")
    print(f"  Combined (n={nm+nw}): Brier = {combined:.5f}")
    print()
    print(f"  vs Stage 32 (single final model): 0.12178")
    print(f"  vs Stage 29 (classifier + market): 0.12287 (local) / 0.12132 (Kaggle)")
    print(f"  delta vs Stage 32: {combined - 0.12178:+.5f}")
    print()
    ...
    out = all_sub[["ID", "Pred"]]
    out_path = SUBMISSIONS / "v2_margin_ensemble.csv"
